...
public class PlanningTrajetService {
    ...
    public void genererPlanning() throws Exception {
        try {
            ...
            // Sprint 7: optimisation avancée générique
            // 1) regrouper les non-assignés par créneau
            // 2) réassigner en priorité selon disponibilité/capacité
            optimiserReservationsNonAssigneesParCreneau();

            // Re-synchroniser les détails après la passe d'optimisation.
            reconstructirePlanningDetailsAprèsAssignation();
        } catch (Exception e) {
    ...
    private void optimiserReservationsNonAssigneesParCreneau() {
        try {
            int maxIterations = 10;
            int iteration = 0;

            while (iteration < maxIterations) {
                List<Reservation> nonAssignees = reservationService.getReservationNonAssignees();
                if (nonAssignees == null || nonAssignees.isEmpty()) {
                    return;
                }

                int avant = nonAssignees.size();

                Map<String, List<Reservation>> groupesParDate = nonAssignees.stream()
                    .filter(r -> r.getDateArrivee() != null && r.getHeure() != null && !r.getHeure().isEmpty())
                    .sorted(Comparator
                        .comparing((Reservation r) -> r.getDateArrivee().toLocalDateTime().toLocalDate())
                        .thenComparing(Reservation::getHeure, Comparator.nullsLast(String::compareTo))
                ...
                for (List<Reservation> reservationsDate : groupesParDate.values()) {
                    reassignerNonAssigneesPourDate(reservationsDate);
                }

                int apres = reservationService.getReservationNonAssignees().size();
                if (apres >= avant) {
                    return;
                }
    ...
    private void reassignerNonAssigneesPourDate(List<Reservation> reservationsDate) throws Exception {
        if (reservationsDate == null || reservationsDate.isEmpty()) {
            return;
        }

        int tempsAttentMaxMinutes = obtenirTempsAttentMaxConfig();
        List<Reservation> restantes = new ArrayList<>(reservationsDate);
        ...
        while (!restantes.isEmpty()) {
            Reservation pivot = restantes.remove(0);
            Vehicule vehicule = trouverMeilleurVehicule(pivot);
            if (vehicule == null) {
                continue;
            }

            int placesRestantes = vehicule.getCapacitePassagers() - pivot.getNombrePersonnes();
            List<Reservation> groupe = new ArrayList<>();
            groupe.add(pivot);

            if (placesRestantes > 0) {
                List<Reservation> candidats = restantes.stream()
                    .filter(r -> peutEtreGroupee(pivot, r, tempsAttentMaxMinutes))
                    .collect(Collectors.toList());

                while (placesRestantes > 0 && !candidats.isEmpty()) {
                    final int placesDisponibles = placesRestantes;

                    Reservation meilleurFit = candidats.stream()
                        .filter(r -> r.getNombrePersonnes() <= placesDisponibles)
                        .min(Comparator
                            .comparingInt((Reservation r) -> Math.abs(placesDisponibles - r.getNombrePersonnes()))
                            .thenComparing(Comparator.comparingInt(Reservation::getNombrePersonnes).reversed())
                            .thenComparingInt(Reservation::getId))
                        .orElse(null);

                    if (meilleurFit == null) {
                        Reservation aFractionner = candidats.stream()
                            .filter(r -> r.getNombrePersonnes() > placesDisponibles)
                            .min(Comparator
                        ...
                        Reservation fragment = reservationService.fractionnerReservation(aFractionner, placesDisponibles);
                        if (fragment == null) {
                            break;
                        }

                        meilleurFit = reservationService.getReservationById(aFractionner.getId());
                        if (meilleurFit == null) {
                            break;
                        }

                        restantes.add(fragment);
                    }

                    groupe.add(meilleurFit);
                    placesRestantes -= meilleurFit.getNombrePersonnes();
                    final int selectedId = meilleurFit.getId();
                    restantes.removeIf(r -> r.getId() == selectedId);
                    candidats.removeIf(r -> r.getId() == selectedId);
                }
            }

            for (Reservation reservation : groupe) {
                assignerVehicule(reservation.getId(), (int) vehicule.getId());
            }

            Reservation derniere = groupe.stream()
                .max(Comparator.comparing(Reservation::getHeure, Comparator.nullsLast(String::compareTo)))
                .orElse(pivot);
            actualiserDisponibiliteVehiculeApresAssignation((int) vehicule.getId(), derniere);
        }
    }
