...
public class PlanningTrajetController {
    ...
    /**
     * Afficher uniquement les réservations non assignées
     */
    @Url("/planning/assignation/non-assignees")
    @Post
    @RestAPI
    public ApiResponse afficherNonAssignees() {
        try {
            List<Reservation> reservations = reservationService.getReservationNonAssignees();
            return ApiResponse.success(reservations.size());
        } catch (Exception e) {
            return ApiResponse.error(500, e.getMessage());
        }
    }
